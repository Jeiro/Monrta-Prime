...
export const AdminSupportTab: React.FC = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} className="space-y-6">
      ...
      {/* Search */}
      <Input type="text" placeholder="Search tickets..." aria-label="Search tickets" prefix={<Search size={16} />} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        ...
        {filtered.map(ticket => {
          ...
          return (
            <div key={ticket.id} className={`bg-surface border rounded-2xl overflow-hidden ${ticket.status === "open" ? "border-negative/30" : "border-line"}`}>
              {/* Raw button on purpose: this is a full-width disclosure region
                  wrapping a card layout (avatar + two text lines + two badges,
                  stacking at sm), not a control. Button forces inline-flex,
                  centred content and a fixed height, which collapses it.
                  aria-expanded added so the toggle state is exposed. */}
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <div className="flex-1 flex gap-2">
                      <div className="flex-1">
                        <Input placeholder="Type a reply..." aria-label="Reply to ticket" value={replyTexts[ticket.id] || ""} onChange={e => setReplyTexts(prev => ({ ...prev, [ticket.id]: e.target.value }))}
                          onKeyDown={e => { if (e.key === "Enter") handleReply(ticket.id); }} />
                      </div>
                      <Button onClick={() => handleReply(ticket.id)} aria-label="Send reply"><Send size={14} /></Button>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2 pt-2 border-t border-line/50">
                    {(["low", "medium", "high"] as const).map(p => (
                      <Button key={p} size="sm" variant={ticket.priority === p ? "primary" : "secondary"} aria-pressed={ticket.priority === p} onClick={() => adminSetTicketPriority(ticket.id, p)}>
                        {p.toUpperCase()}
                      </Button>
                    ))}
                    <Button size="sm" variant="positive" onClick={() => { adminCloseTicket(ticket.id); setFeedback("Ticket closed."); setTimeout(() => setFeedback(null), 3000); }}>
                      Mark Resolved
                    </Button>
                  </div>
                </motion.div>
